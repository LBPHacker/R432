...
local function build(params, params_name, component)
	...
	do
		...
		aray(x_r3_1 + 6, y_r3_1 - 2, 1, 0, pt.METL, nil, 1)
		part({ type = pt.STOR, x = x_r3_1 + 5, y = y_r3_1 - 2 })
		part({ type = pt.FILT, x = x_r3_1 + 4, y = y_r3_1 - 2 })
		part({ type = pt.FILT, x = x_r3_1 + 3, y = y_r3_1 - 2, tmp = 1, ctype = 0x1000FFFF })
		part({ type = pt.FILT, x = x_r3_1 + 2, y = y_r3_1 - 2, tmp = 7, ctype = 1 })
		part({ type = pt.BRAY, x = x_r3_1 + 1, y = y_r3_1 - 2, life = 1 })
		...
		part({ type = pt.DTEC, x = x_second + 1, y = y_r3_1, tmp2 = 2 })
		dray(x_second + 1, y_r3_1, target_b.x, target_b.y, 1, pt.METL)
		ldtc(x_second + 1, y_r3_1, source_2_b.x, source_2_b.y)
		aray(x_second + 1, y_r3_1, -1, 0, pt.METL, nil, 1)

		part({ type = pt.FILT, x = x_second +  2, y = y_r3_1, tmp =  7 })
		part({ type = pt.STOR, x = x_second +  3, y = y_r3_1 })
		...
		part({ type = pt.STOR, x = x_second + 18, y = y_r3_1 })
